...
コンポーネント仕様の正本は `materials/Design/components_spec.md` とし、本書では重複定義を持たない。
...
> **ルート `/`・タイムゾーン・案件詳細 `detailTab`・サイドバー／プロフィールのロール表示**は **`materials/Design/system_spec.md`（URL 設計・ナビゲーション）** を正とする。
