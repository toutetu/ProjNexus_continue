...
| **予算管理** | 案件一覧（予算タブ） / 案件詳細（予算） / 予算実績入力 | 申請者（主担当）・管理者（閲覧） |
...
| S-11 | 予算実績入力モーダル | `S-04（detailTab=budget）内`（深い導線: `/projects/{id}/budget-input`） | 予算管理 | actual_amount の入力 |
| S-12 | 通知一覧 | `/notifications` | グローバル | アプリ内通知 |
| S-13 | プロフィール | `/profile` | グローバル | アカウント参照（氏名・部署・ロール・メール）＋本人入力（任意）（実装は `Profile/Edit.tsx`。パスワード変更UIは本画面外） |
...
テストアカウントの最新情報は固定記載せず、`database/seeders/UserSeeder.php` を正とする（運用・検証シナリオは提出用 **`doc/Information.md`** を参照）。
...
| S-14 タスク一覧（board 既定）| 自部門の **承認済み案件** のタスク一覧（閲覧は `ProjectWorkItemPolicy::view`、編集は主担当・担当・確認者に準拠） | 自部門の全タスク（既定は members）| 部門選択時、選択部門の全タスク（**閲覧のみ**・既定は members）※HQ |
| S-14 タスク一覧（members）| 自部門の全タスク（**閲覧のみ**）| 自部門の全メンバー × 全タスク | 部門選択時、同上（**本部は閲覧のみ**）※HQ |
| S-14 タスク一覧（list）| 自部門の **承認済み案件** のタスク一覧（board と同じスコープ） | 自部門の全タスク | 部門選択時、同上（**本部は閲覧のみ**）※HQ |

※ **S-10 申請者列**: タスクの編集は主担当に加え、当該タスクの担当・確認者にも及ぶ（`ProjectWorkItemPolicy::canEditTaskContent`）。**一覧（S-03）**のクエリは `Project::scopeVisibleTo` と単体 `view` で条件が異なる場合がある（`materials/Design/role_feature_matrix_.md` の照合節参照）。  
※HQ **本部管理者**はタスクを **閲覧のみ**（モーダルでの保存・DnD・コメント投稿・再オープン等の書き込みは不可）。**方針（2026-05-12）**・実装は `materials/daily_reports/implementation_schedule.md` §3 の必須項。現行コードは未反映の可能性あり。
...
- 導線は `S-04（予算タブ）` の「実績を入力」ボタン（モーダル表示）
- `/projects/{id}/budget-input` は `S-04（detailTab=budget）` を開いてモーダルを表示する深い導線として扱う
- サイドバーは `予算状況一覧 → ↳案件詳細 → ↳予算実績入力` を1つの merged ハイライトで表示
