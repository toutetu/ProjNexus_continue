...
> **設計判断の背景** は `materials/Design/design-philosophy.md` を参照
（内容は重複させないため、思想側はストーリー中心のまま残す）。  
> **テーブル列・Enum の網羅** は `materials/Design/er_diagram.md` を参照。
...
- **`materials/Design/`**: 設計資料（編集の正本）
- **`materials/daily_reports/`**: 日報・進捗（編集の正本）
- **`doc/`**: 提出指定ファイルのみ（レビュー対象。それ以外は置かない方針）
...
- 履歴・更新時刻の表示は **日本時間（JST）** を基準とする
- `config/app.php` の `timezone` は `env('APP_TIMEZONE', 'Asia/Tokyo')`
- 変更後は `php artisan config:clear` で設定キャッシュをクリアする
...
- URL 形: `/projects/{id}?detailTab={apply|tasks|budget|history}`
- 案件一覧から遷移したときの初期タブ（実装）: 申請タブ流入 → `apply`、開発タブ → `tasks`、予算タブ → `budget`（`resources/js/Pages/Projects/Index.tsx` の `showDetailHref` 等を正とする）
- 互換: `?detailTab=overview` は **`apply` と同義**（`resources/js/Pages/Projects/Show.tsx` の `parseDetailTab`）
...
**サイドバー・ユーザーカードのロール説明文**（2026-05-12）: サイドバー下部のユーザーカードで、ログイン中ユーザーに次の補足文を表示する（表示位置は氏名および `ロール名 / 部門名` の下段）。

- 申請者: 「案件の申請・進捗/予算入力を担当」
- 部門管理者: 「自部門案件の承認と運用を担当」
- 本部管理者: 「全社案件の最終承認と全体閲覧を担当」

**プロフィール（`/profile`）**: 氏名・部署・メール・**ロール**（複数ロールは Spatie のロールを日本語ラベル化し、**` / `（スペース・スラッシュ・スペース）** で連結。例: `申請者 / 部門管理者`）を「アカウント情報（表示のみ）」ブロックに読み取り専用で表示。本人入力（生年月日・性別）は任意編集可。**パスワード変更の UI は本画面に含めない**（簡素化方針）。アカウント削除導線は非表示。
...
| 資料 | パス |
|------|------|
| AI 入口（作業ルール・モック一覧） | `materials/Design/AI.md` |
| 要件原文 | `materials/quest/requirements.md` |
| 設計の背景（プレゼン） | `materials/Design/design-philosophy.md` |
| 画面遷移 | `materials/Design/screen_flow.md` |
| ER・区分値 | `materials/Design/er_diagram.md` |
| デザイントークン | `materials/Design/design_system.md` |
| コンポーネント Props | `materials/Design/components_spec.md` |
| ロール×機能 | `materials/Design/role_feature_matrix_.md` |
| デプロイ・検証手順（提出・レビュー用） | `doc/Information.md` |
| 次回作業・マスト改修 | `materials/daily_reports/implementation_schedule.md` |
