...
マスト改修の**チェック項目単位の進捗**は本ファイル §「Phase 5 手動確認・マスト改修」に集約します。`implementation_schedule.md` は明日以降の作業目線の要約のみとします。
...
- **現在の主作業:** Phase 5（資料同期・提出準備・デプロイ確認）。マスト改修のチェックは **本ファイル §「Phase 5 手動確認・マスト改修」** を正とする
...
- **直近の実装（2026-05-12 追記）:** マスト #12（`ScenarioMirrorSeeder`・`track-b-*` / `track-c-*`）、マスト #13（申請者 S-14 board/list の部門内タスク閲覧）。マスト #8 はスコープ外（ユーザー方針）
...
- **次の着手:** Phase 5（資料・スクショ・提出チェック）。マスト改修の残: 手動チェックリストの #5/#8（#8 はスコープ外化）、サイドバー調査メモの要否判断 等
- **次の着手（具体）:** 変更後 UI のスクショ差し替え（予算実績入力ページとサイドバー3行 merged含む）、S-14 DnD 修正の再確認、マニュアル・プレゼンへの反映

---

### Phase 5 手動確認・マスト改修（`implementation_schedule.md` §3 より転記・2026-05-12）

> ここをチェック項目の正として更新する。実装スケジュール要約版は明日以降の作業目線のみ記載。
...
- [ ] **PoC 採点用:** デプロイ手順に DB 初期化（例: `php artisan migrate:fresh --seed`）を記載し、採点者が手順どおり再現できること（未充足なら日報に明記）
- [ ] プレゼン提出物（`.md` または `.pdf`）の最終版を確定
- [ ] マスト改修の実施結果を日報・設計書へ反映（上表どおりチェック）
...
6. 提出 README に **`migrate:fresh --seed`** を明記（採点者向け）
...
- [x] ロールの説明を画面上で分かるようにする（2026-05-12: サイドバーのユーザーカードに役割説明を追加）
- **~~申請者がスマホで確認できるようにする~~** → **マスト #8 スコープ外**（2026-05-12。PC 優先のため未実装）
