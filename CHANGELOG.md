...
## [Unreleased](https://github.com/laravel/laravel/compare/v2026.04.27-phase2...main)

## [v2026.04.27-phase2](https://github.com/laravel/laravel/compare/v12.12.1...v2026.04.27-phase2) - 2026-04-27

### Phase 2 Complete (2026-04-27)

#### 概要

Phase 2（申請・承認フロー）を完了しました。
申請〜承認〜却下〜再申請の主要導線、通知連携、却下時の情報表示をモックに準拠して作成しました。。
...
- 申請取り戻し機能
  ...
  - 部門管理者申請者は `pending_hq`（本部直行）でも取り戻し可能
  
- 却下案件詳細で却下コメントを表示
  
  - 承認ステップ下に注意表示（薄い赤背景）
  - 却下ステータス案件は閲覧ロール共通で表示
  

##### 2. 通知機能

- 申請時に承認者へ通知（部門管理者 / 本部管理者）
- 部門承認後に本部管理者へ承認依頼通知
- 本部却下時に、途中承認した部門管理者へ却下通知
- 申請者向け「申請受付」通知を即時既読化

##### 3. 一覧・UI改善

- 承認待ち件数バッジをサイドバー表示（0件は非表示）
- ヘッダー右上の検索窓を全画面から削除
- 却下案件詳細表示時のサイドバーアクティブを申請一覧へ切替
- ログイン画面テストユーザー欄に共通パスワード表示を追加

#### 検証

- TypeScript 型チェック: `npx tsc --noEmit` 成功
- Feature Test: `ProjectApprovalFlowTest` 成功
  - 承認・却下・通知・取り戻しの主要シナリオを確認
  

#### 既知の補足

- 日次ドキュメント（daily系）は運用ルールに従って更新済み
- 次フェーズは Phase 3（開発/予算タブの接続）を予定
