...
| Phase 4 | +α（最小） | 15h | 5h | ✅ 完了（S-14 3ビュー・4値運用・通知拡張・設計同期） |
| Phase 5 | 資料・マニュアル・最終確認 | 20h | 1h | 🟡 進行中 |
| 予備バッファ | 遅延吸収・回帰修正 | 0h |  - | - |
...
- **現在の主作業:** Phase 5（資料同期・提出準備・デプロイ確認）
- **直近完了:** S-14（board/members/list）、4値運用（`resolved`/`closed`）、通知拡張（`task_resolved`/`task_reviewed`）、設計書群の実装同期
- **直近完了（設計）:** `er_diagram.md` v6 化、`components_spec.md` 正本化、`Information.md` シナリオ拡充、`screen_flow.md` 実装整合、`s16_burndown_policy.md` 追加
- **次の着手:** マニュアル最終化、プレゼン資料確定、提出物チェックの実体確認
...
日次の git 手順・コミット方針・チェックリストは **`doc/Design/AI.md §9 日次ワークフロー` を唯一の正本**とする。本ファイルには手順を記載しない（二重管理を避けるため）。
...
### S-03a（`mockups/s03a_policy.md`）と実装の整合（2026-05-02 反映）
...
### 継続UI調整（実施中）
- [ ] ログイン画面に部門長テストユーザーをプリセット
- [ ] ログイン画面で3ロール代表ユーザー表示＋「＋」で全ユーザー展開
- [ ] ログイン画面のマニュアルリンクをテストユーザー一覧より上へ配置
- [ ] 全マニュアルリンクを別窓表示に統一
- [ ] アプリ名を「開発管理アプリ projnexus」に変更
- [ ] サイドバー左上アプリ名クリックで Notion へ遷移
- [ ] サイドバーのプロフィール上に「マニュアル」導線を追加
- [ ] 管理画面サイドバー見出し（JPT 開発管理）の縦パディング調整
- [ ] レフトナビ開閉（折りたたみ）機能、幅調整機能の追加
- [ ] フォーム補足説明のツールチップ化（常時表示文を整理）
- [ ] 部門管理者向け承認フロー説明を `ApprovalStepperFull` へ置換し、設計方針書・`doc/Design` も同期
- [ ] マニュアル 2.2 承認フロー（状態遷移図）のラベル配色可読性を改善（黒地/白字）
- [ ] マニュアルリンク切れを総点検
- [ ] GitHub プロジェクト名を `projnexus` に変更
- [ ] `/projects/3?detailTab=apply` 承認画面に申請画面同等の添付ファイル欄を追加
/**更新完了**/