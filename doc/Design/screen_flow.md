...
> - **課題2（+α・企画力の提示）**: ダッシュボード、予算リアルタイム可視化、承認ステップ表示などは「テーブル設計」と「プレゼン」で示す。実装はインターン後のポートフォリオ拡張フェーズに回す
...
| S-14 | タスク一覧 | `/member-tasks?view={board\|members}` | 開発管理 | 部門メンバーのタスク状況を俯瞰（4値運用・カンバン/メンバー別の2ビュー） |
...
| S-11 予算実績入力 | 自案件の主担当 | × | × |
| S-14 タスク一覧（board 既定）| 自分が `assignee_id` or `reviewer_id` のタスク | 自部門の全タスク（既定は members）| 部門選択時、選択部門の全タスク（既定は members）|
| S-14 タスク一覧（members）| 自部門の全タスク（**閲覧のみ**）| 自部門の全メンバー × 全タスク | 部門選択時、同上 |
...
案件一覧の**どのタブから入ったか**で、S-04 内の初期表示タブを変える。`components_spec.md` のモーダル文脈（S-10 はタスクタブ dim、S-11 は予算文脈 dim）と一致させる。

| 流入元 | 行クリック後の S-04 初期タブ | URL クエリ案（実装で確定） |
|--------|------------------------------|-----------------------------|
| S-03 申請タブ | 概要（承認状況・案件情報が一覧の文脈に近い） | `/projects/{id}` または `?detailTab=overview` |
| S-03 開発タブ | **タスク** | `?detailTab=tasks` |
| S-03 予算タブ | **予算** | `?detailTab=budget` |
| S-07 承認待ち（S-03a プリセット） | 概要（承認ステッパーと承認アクションカードが見える） | `?detailTab=overview` |

**補足**:
- クエリ名 `detailTab` は仮称。ルート設計時に `tab` / `section` などプロジェクト内で一本化する。
- S-04 のタブ構成は `概要 / タスク / 予算 / 履歴`（§7 参照）。`history` タブの中で「承認履歴」「変更履歴」を2サブセクションで並列表示する。
- タスク由来の変更履歴（`task_histories`）は **履歴タブのタイムライン** に案件イベントとして合流するほか、**タスクタブ** の一覧行を展開してタスク単位でも閲覧できる（同一データ）。
- `detailTab` の値は `overview | tasks | budget | history` の4種で固定。
...
1. **開発管理 → タスク一覧（S-14・メンバー別ビュー初期）** で部門選択して全部門のタスク横断把握
...
- **タブ切替**: 概要 / タスク / 予算 / 履歴 など（モックに合わせて確定）。一覧からの遷移時は §4-4 の初期タブを適用する
...
- **ビュートグル**: カンバン / メンバー別（URLクエリ `?view=board|members` で状態保持、ピル型トグル）
- **ロール別の初期ビュー**: applicant=board / dept_manager・hq_manager=members
- **フィルタ**: 部門（dept_managerは自部門固定 disabled、hq_managerは選択必須）/ 担当者 / 案件 / 優先度 / 期日（期限超過・3日以内・今週・今月・期日未設定）
...
- **メンバー別ビュー（members）**: 行=メンバー、列=3列（未着手 / 進行中 / 完了）
  - `resolved` タスクは完了列に「確認待ち」アイコン付き紫枠で併記（独立列にはしない）
  - メンバー情報: アバター・氏名・ロール・サマリバッジ（超過 N / 3日以内 N / 順調）・**負荷バー（担当数 / 8件固定）**
  - ソート切替: 期限超過多い順（既定）/ 担当数多い順 / 進行中多い順 / 名前順
  - **KPI ストリップ**: 部門合計 / 期限超過 / 3日以内 / 完了率
- **アクセス制御**: members ビューは全ロール閲覧可。閲覧 OK / 編集 NG を Policy で分離
- **動線**: カードクリック → タスク編集モーダル（S-10 流用）/ 案件名クリック → S-04 案件詳細
...
- S-03 各タブから S-04 へ行くときの**初期表示タブ**は §4-4 に従う（開発→タスク、予算→予算、申請→概要）
- S-04 のタブ構成は `概要 / タスク / 予算 / 履歴`。`history` タブ内で承認履歴・変更履歴を上下に並列表示する（タブ数を増やさない）。タスクの変更内容は `task_histories` を参照
