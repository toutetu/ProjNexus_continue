...
| **開発管理** | 案件一覧（開発タブ） / 案件詳細 / タスク一覧（board/members/list） / タスク入力 | 申請者（主担当）・管理者 |
...
| S-14 タスク一覧（list）| 自分が `assignee_id` or `reviewer_id` のタスク | 自部門の全タスク | 部門選択時、同上 |
