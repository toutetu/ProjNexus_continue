...
1. [はじめに](#1-はじめに)
2. [前提：5つの状態と承認フロー](#2-前提5つの状態と承認フロー)
3. [共通操作](#3-共通操作)
4. [申請者向けガイド](#4-申請者向けガイド)
5. [部門承認者向けガイド](#5-部門承認者向けガイド)
6. [本部承認者向けガイド](#6-本部承認者向けガイド)
7. [状態と規則（リファレンス）](#7-状態と規則リファレンス)
...
| タスクの作成・編集 | ○（承認済かつ自分が主担当） | ○（自部門の承認済案件） | ○（全承認済案件） |
| 予算実績の入力 | ○（承認済かつ自分が主担当） | ○（自部門の承認済案件） | ○（全承認済案件） |
...
タスク・予算実績の入力自体は本部管理者でも可能ですが、運用上は主担当が入力するのが基本です。
...
タスクの作成・編集は **承認済の案件のみ** 可能です。下書き・承認待ち・却下中の案件にはタスクタブ自体が表示されません。承認済案件であれば、自分が主担当（または自部門・本部管理者）である必要があります。
