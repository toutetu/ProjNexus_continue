...
## 8. 次回作業の具体案（Phase 0完了回）

### 目的
- 環境構築を完了し、ロール付きでログイン後画面まで安定動作させる

### 実行手順
1. `spatie/laravel-permission` 導入状態を確認（未完なら導入）
2. Permission関連のmigrationを公開・実行
3. Department / Role / User のSeederを作成
4. `migrate:fresh --seed` で初期データ投入
5. ログイン後遷移を `projects?tab=approval` に固定
6. 3ロールでログイン検証（表示・遷移・権限の最小確認）
7. 日報更新（完了事項・詰まった点・次回タスク）

### 判断理由
- 権限設計（Spatie）と初期データ（Seeder）を先に固めると、以降の画面実装で検証が速くなる
- ログイン後遷移を先に固定すると、毎回の動作確認導線が安定する
- Feature開発前にロール別ログイン確認をしておくことで、権限系の手戻りを減らせる

### 完了条件
- Role/Permission/Department/User のseedが投入される
- 3ロールでログインできる
- ログイン後に `projects?tab=approval` へ遷移する
- 最低限の権限制御（表示/アクセス）が確認できる

### 保留・課題（想定）
- Policy詳細はPhase 2着手時に画面単位で厳密化
- UIの最終調整はPhase 1-3で段階的に実施
